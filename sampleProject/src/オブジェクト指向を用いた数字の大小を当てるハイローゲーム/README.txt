...
The game generates a random number between 1 and 9, and the player has to guess
whether the next randomly generated number will be bigger or smaller than the previous one.
...
that generates random numbers, a Player class that handles player input, and a BigOrSmall class that runs the game logic.
There is also a custom NumberOutOfRange_Exception class for handling exceptions related to player input.

The game starts by displaying the current number and asking the player to guess whether the next number will be bigger or smaller.
